...
class ChatSearchService(
    ...
    private fun validateChannelsBatch(channels: List<ChatSearchResult>, query: String): Set<Int> {
        ...
        val prompt = """
Ты — строгий фильтр Telegram-каналов и групп. Запрос пользователя: "$query"

Список каналов/чатов для оценки:
$channelList

Для каждого канала реши: KEEP (оставить) или REMOVE (удалить).

KEEP только если:
- Тематика соответствует запросу пользователя (прямо или косвенно — профессиональная аудитория)
- Это активный чат, где люди ищут/предлагают услуги по теме
- Нет признаков NSFW, спама, мошенничества

REMOVE если:
- Эротика, 18+, adult-контент, NSFW
- Казино, ставки, крипто-скам, финансовые пирамиды
- Совершенно не по теме запроса
- Только новостной канал без обсуждений (peer_type = channel без интерактива)
- Спам, боты, накрутка, реклама
