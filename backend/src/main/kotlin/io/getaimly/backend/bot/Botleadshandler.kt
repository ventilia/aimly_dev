...
class BotLeadsHandler(
    ...
    fun showLeadsMenu(chatId: Long, tgUserId: Long, msgId: Int? = null) {
        ...
        if (msgId != null) sender.editText(chatId, msgId, text, kb, parseMarkdown = true)
        else sender.sendText(chatId, text, kb, parseMarkdown = true)
    }
    ...
    fun showLeadsList(chatId: Long, msgId: Int, tgUserId: Long, page: Int, statusFilter: String?) {
        ...
        sender.editText(chatId, msgId, sb.toString().trimEnd(), InlineKeyboardMarkup(rows), parseMarkdown = true)
    }
