...
export default function DashboardOverview({ lang }: Props) {
    ...
    // После бэкенд-фикса (tgNotifiedAt в DTO) unratedNotified определяется корректно.
    ...
    useEffect(() => {
        ...
        Promise.all([
            ...
            // Работает корректно только если бэк возвращает tgNotifiedAt в DTO.
            ...
            setLastLead(target)
            // userRating теперь приходит с бэка — инициализируем состояние из него
